...
public class DBUtil {
    ...
    private void initDB() throws SQLException {
        ...
        statement.execute("CREATE TABLE PORTFOLIOS (USERID VARCHAR(50) NOT NULL, TICKER VARCHAR(10) NOT NULL, SHARES INT NOT NULL, PRICE DOUBLE NOT NULL, PRIMARY KEY(TICKER,USERID))");
        statement.execute("CREATE TABLE STOCKTRANSACTIONS (TRANSACTION_ID INTEGER NOT NULL GENERATED ALWAYS AS IDENTITY (START WITH 4022, INCREMENT BY 1), ACCOUNTID BIGINT NOT NULL, DATE DATE NOT NULL, TICKER VARCHAR(10) NOT NULL, NAME VARCHAR(50) NOT NULL, SHARES INT NOT NULL, ACTION VARCHAR(10) NOT NULL, TYPE VARCHAR(100) NOT NULL, PRICE DOUBLE NOT NULL, PRIMARY KEY (TRANSACTION_ID))");
...
//			Connection connection = getConnection();
//			Statement statement = connection.createStatement();
////        statement.execute("DELETE FROM PEOPLE WHERE USER_ID = 'admin'");
...
//			shares are negative for buy orders
            ...
            if (tradeSpent < 0) {
                ...
                statement.execute("INSERT INTO STOCKTRANSACTIONS (ACCOUNTID, DATE, TICKER, NAME, SHARES, ACTION,TYPE, PRICE) VALUES (" + cashId + ",'" + sqlDate + "','" + ticker + "','" + stockName + "'," + (-shares) + ",'BUY','" + orderType + "'," + currentPrice + ")");
//				statement.execute("INSERT INTO PORTFOLIOS (ACCOUNT_ID,TICKER,SHARES) VALUES ("+cashId+",'"+ticker+"',"+shares+")");
                ...
                statement.execute("INSERT INTO STOCKTRANSACTIONS (ACCOUNTID, DATE, TICKER, NAME, SHARES, ACTION,TYPE, PRICE) VALUES (" + cashId + ",'" + sqlDate + "','" + ticker + "','" + stockName + "'," + shares + ",'SELL','" + orderType + "'," + currentPrice + ")");
            }
...
//                statement.execute("UPDATE PORTFOLIOS SET SHARES = " + newPosition.getShares() + ", PRICE = " + newPosition.getPrice() + " WHERE TICKER = '" + newPosition.getTicker() + "' AND USERID = '" +username + "'");
//                System.err.println("update portfolio: " + ticker + ": current shares " + newPosition.getShares());
//            }
...
//				statement.execute("UPDATE PORTFOLIOS SET SHARES = "+newPosition.getShares()+", PRICE = "+ newPosition.getPrice() + "WHERE TICKER= '" + newPosition.getTicker()+"'");
          System.out.println(updatePortfolio(port,newPosition,statement));
//				System.out.println("update portfolio with existing stock");
			}else {
          port.add(p);
//				statement.execute("INSERT INTO PORTFOLIOS (USERID,TICKER,SHARES,PRICE) VALUES ('"+username+"','"+p.getTicker()+"', "+p.getShares()+", "+ p.getPrice()+")");
          System.out.println(addNewPosition(p,username,statement));
//				System.out.println("update portfolio with new stock");
    ...
    public static String updatePortfolio(Portfolio port, Position p,Statement statement) {
        try {
            checkAllTables();
            Position newPosition = port.getPositions().get(p.getTicker());
            statement.execute("UPDATE PORTFOLIOS SET SHARES = " + newPosition.getShares() + ", PRICE = " + newPosition.getPrice() + "WHERE TICKER= '" + newPosition.getTicker() + "'");
    ...
    public static String addNewPosition(Position p, String username,Statement statement) {
        try {
            checkAllTables();
            statement.execute("INSERT INTO PORTFOLIOS (USERID,TICKER,SHARES,PRICE) VALUES ('" + username + "','" + p.getTicker() + "', " + p.getShares() + ", " + p.getPrice() + ")");
            System.out.println("update portfolio with new stock");
...
//		System.out.println("Complete creating portfolio");
    ...
    public static List<Position> getAllPosition() {
        ...
        try {
            ...
            String query = "SELECT TICKER, SUM(SHARES) AS SHARES, AVG(PRICE) AS PRICE FROM PORTFOLIOS GROUP BY TICKER ";
            ResultSet resultSet = statement.executeQuery(query);
//            System.out.println("finished query");

            while (resultSet.next()) {
                String ticker = resultSet.getString("TICKER");
                System.out.println("all position stock:" + ticker);
                int shares = resultSet.getInt("SHARES");
                System.out.println("all position shares: " + shares);
                double price = resultSet.getDouble("PRICE");
                System.out.println("all position price: " + price);
                Position p = new Position(ticker, shares, price);
                positions.add(p);
            }
    ...
    public static List<Position> getTodayBUY() {
        List<Position> positions = new ArrayList<>();
        try {
            Connection connection = getConnection();
            Statement statement = connection.createStatement();
            ...
            String query = "SELECT TICKER, SUM(SHARES) AS SHARES, ACTION FROM STOCKTRANSACTIONS WHERE DATE = '"+ date +"' GROUP BY TICKER, ACTION";
            ResultSet resultSet = statement.executeQuery(query);
            System.out.println("finished buy query");

            while (resultSet.next()) {
                String ticker = resultSet.getString("TICKER");
                System.out.println("today buy stock:" + ticker);
                int shares = resultSet.getInt("SHARES");
                System.out.println("today buy shares: " + shares);
                String action = resultSet.getString("ACTION");
                if(action.equals("BUY")){ //buy is negative
                    shares = -shares;
                }
                Position p = new Position(ticker, shares, 0);
                positions.add(p);
            }
    ...
    public static List<Position> getTodaySELL() {
        List<Position> positions = new ArrayList<>();
        try {
            Connection connection = getConnection();
            Statement statement = connection.createStatement();
            ...
            String query = "SELECT TICKER, SUM(SHARES) AS SHARES FROM STOCKTRANSACTIONS WHERE DATE = '"+ date +"' AND ACTION = 'SELL' GROUP BY TICKER ";
            ResultSet resultSet = statement.executeQuery(query);
            System.out.println("finished sell query");

            while (resultSet.next()) {
                String ticker = resultSet.getString("TICKER");
                System.out.println("today sell stock:" + ticker);
                int shares = resultSet.getInt("SHARES");
                System.out.println("today sell shares: " + shares);
                Position p = new Position(ticker, shares, 0);
                positions.add(p);
            }
